...
def set_status_inactive(set_ID):
    connection = establish_db_connection('owner')
    cursor = connection.cursor()
    inactive = 'Inactive'
    statement = 'UPDATE ALLEGRO_OWNER.SEARCH SET STATUS = :inactive WHERE SETID = :setid'
    cursor.execute(statement, setid = set_ID, inactive = inactive)
